...
class VendorsInvoices(QWidget):
    def __init__(self, engine, parent=None):
        ...
        filterbox_line2.addWidget(label_vendor)
        filterbox_line2.addWidget(self.filter_vendor)
        ...
        self.table = ATable()
        self.table.setModel(VendorsInvoicesModel(self.table, self.engine))
...
class VendorsInvoicesModel(ATableModel):
    ...
    def _do_requery(self):
        ...
        vendors = pd.read_sql_query("SELECT id, name FROM B05_Vendors", self.engine)
        ...
        # Merge doctypes and vendor
        doctypes.rename(columns={"id": "type_id", "name": "type_name"}, inplace=True)
        vendors.rename(columns={"id": "vendor_id", "name": "vendor_name"}, inplace=True)
        invoices.rename(columns={"id": "document_id"}, inplace=True)
        invoices = pd.merge(invoices, doctypes, on="type_id", how="left")
        invoices = pd.merge(invoices, vendors, on="vendor_id", how="left")

        filter_vendor = self.FILTER.get("vendor")
        filter_definition = self.FILTER.get("definition_id")
        filter_number = self.FILTER.get("number")

        if filter_vendor:
            invoices = invoices[invoices["vendor_name"].str.contains(filter_vendor, case=False)]
        ...
        invoices = invoices.reindex(columns=["document_id", "type_name", "number", "vendor_name",
                                                 "date", "date_due", "date_planned_clearing", "date_cleared",
                                                "cleared", "description",
                                                "amount", "currency", "remaining_amount",
                                                "priority", "void", "memo"])

        invoices.columns = ["id", "Tips", "Numurs", "Piegādātājs",
