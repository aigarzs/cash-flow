...
class InvoiceClearingModel(ATableModel):
    def _do_requery(self):
        ...
        clearing = pd.read_sql_query("SELECT * FROM H04_Reconciliations_DR WHERE cr_docid = " + str(invoice_id), self.engine)
        clearing["date"] = pd.to_datetime(clearing["date"])
        clearing = clearing.reindex(columns=["dr_docid", "type_name", "number", "date",
                                             "description", "partner_name",
                                             "r_amount", "currency", "dr_amount"])
        clearing.columns = ["dr_docid", "Dok. Tips", "Numurs", "Datums",
                            "Apraksts", "Partneris",
                            "Summa", "Valūta", "Dokumenta Summa"]
        clearing.set_index("dr_docid", inplace=True)
...
class CustomersInvoicesModel(ATableModel):
    ...
    def _do_requery(self):
        ...
        acctype = str(AccountType.VENDORS_ACCOUNT)
        cash_type = "Payment"
