...
class Browse_BudgetedPayment(QWidget):
    def __init__(self, engine, return_method):
        ...
        self.label_name = QLabel("Budžeta maksājumi")
        self.table = ATable()
        self.table.setModel(BudgetedPaymentModel(self.table, self.engine, label_amount))
...
class BudgetedPaymentModel(ATableModel):
    ...
    def _do_requery(self):
        ...
        transactions = pd.read_sql_query("SELECT * FROM F01_BudgetEntries WHERE date > '" + previous_day_date_from +
        ...
        # print(definition)
        transactions = transactions[transactions["definition_id"] == definition_id]
        ...
        transactions["cash_status"] = "Budgeted"
        ...
        transactions = transactions.reindex(columns=["id", "period_end", "cf_type_translated", "cf_status_translated", "memo", "date", "amount_LC"])
        transactions.columns = ["id", "Periods", "NP Tips", "NP Statuss", "Piezīmes", "Datums", "Summa BV"]
        transactions = transactions.set_index("id")

        total = (-1 * transactions["Summa BV"]).sum()
