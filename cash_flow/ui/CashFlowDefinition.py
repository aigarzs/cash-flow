...
from cash_flow.database.Model import CashFlowDefinitionAccounts as CFAccounts
from cash_flow.database.Model import CashFlowDefinitionTotals as CFTotals
...
class TotalsModel(ATableModel):
    ...
    def _do_requery(self):
        stmt = select(CFTotals.id,
                      CFTotals.operator,
                      CFDefinition.name) \
            .join(CFDefinition, CFTotals.definition_summarized==CFDefinition.id) \
            .filter(CFTotals.definition_id == self.definition_id)
    ...
    def _generate_default_row(self):
        ...
        """
        with Session(self.engine) as session:
            totals = CFTotals(definition_id=self.definition_id)
            session.add(totals)
            session.commit()
    ...
    def _set_data_in_database(self, index, value):
        ...
        with Session(self.engine) as session:
            stmt = select(CFTotals).where(CFTotals.id == totals_id)
    ...
    def _delete_row_in_database(self, row):
        ...
        with Session(self.engine) as session:
            stmt = delete(CFTotals).where(CFTotals.id == totals_id)
...
class AccountsModel(ATableModel):
    ...
    def _do_requery(self):
        stmt = select(CFAccounts.id,
                      CFAccounts.operator,
                      CFAccounts.entry_type,
                      CFAccounts.account) \
               .order_by(CFAccounts.account)

        stmt = stmt.filter(CFAccounts.definition_id == self.definition_id)
    ...
    def _generate_default_row(self):
        ...
        """
        with Session(self.engine) as session:
            accounts = CFAccounts(definition_id=self.definition_id)
    ...
    def _set_data_in_database(self, index, value):
        ...
        with Session(self.engine) as session:
            stmt = select(CFAccounts).where(CFAccounts.id == accounts_id)
    ...
    def _delete_row_in_database(self, row):
        ...
        with Session(self.engine) as session:
            stmt = delete(CFAccounts).where(CFAccounts.id == accounts_id)
            session.execute(stmt)
            session.commit()
